...
void Core1a_loop() {
  ...
  float joy_x = (float)remote_data.joy_lx;
  float joy_y = (float)remote_data.joy_ly;
  float speed_norm = sqrtf(joy_x * joy_x + joy_y * joy_y);
  if (speed_norm > 100.0f) speed_norm = 100.0f;

  int16_t move_dir = 0;
  if (speed_norm > 1.0f) {
    move_dir = (int16_t)(atan2f(joy_x, joy_y) * 180.0f / M_PI);
  }
  int16_t move_dir_half = move_dir / 2 + 90;
  if (move_dir_half < 0) move_dir_half = 0;
  if (move_dir_half > 180) move_dir_half = 180;

  uint8_t move_speed = (uint8_t)speed_norm;
  bool do_kick = remote_data.button_a;
  bool do_dribble = remote_data.button_b;
  bool do_direct_kick = remote_data.button_x;

  send_byte[0] = 0xFF;
  send_byte[1] = yaw / 2 + 90;
  send_byte[2] = ir.GetDir() / 2 + 90;
  send_byte[3] = ir.GetDis();
  send_byte[4] = (uint8_t)move_dir_half;
  send_byte[5] = move_speed;
  send_byte[6] = (do_direct_kick << 2) | (do_dribble << 1) | do_kick;
  send_byte[7] = 0xAA;
